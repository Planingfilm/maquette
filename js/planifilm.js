...
// Carré de sable JavaScript
  window.onload = function () {
    ...
    var chart = d3.select("#data-figure").insert("figure.svgbulle")
      .append("svg")
      .attr("width", width)
      .attr("height", height)
...
//					}).select("image"));
    ...
    function ticked() {
      ...
      chartImg.selectAll("image")
        .attr('x', function (d) {
          ...
          var myRadius = r(d.priorite) * 1.1;
          return d.x = Math.max(myRadius, Math.min(width - myRadius, d.x));
        })
        .attr('y', function (d) {

          var myRadius = r(d.priorite) * 1.1;
          return d.y = Math.max(myRadius + headerheight, Math.min(height - myRadius, d.y));
        });
      ...
      chartHit.selectAll("circle")
        .attr('cx', function (d) {
          ...
          var myRadius = r(d.priorite) * 1.1;
          return d.x = Math.max(myRadius + headerheight, Math.min(width - myRadius, d.x));
        })
        .attr('cy', function (d) {
    ...
    d3.selectAll(".filter").on("click", function () {
      ...
      // if mine is active
      if (selected == "all" || myButton.attr("class") == "active") {
      ...
      } else // if mine is inactive
      {
        ...
        $(".active").removeClass("active").addClass("inactive");
        // make mine active
        $("#"+selected).addClass( "active").removeClass("inactive");
        // hide all
        chart.selectAll(".bulle")
          .attr("display", "none")
          // show mine
  ...
  /*=========================================================
      Le code JavaScript ci-dessus est basé sur ces exemples:
      https://bl.ocks.org/d3noob/bdf28027e0ce70bd132edc64f1dd7ea4
      https://bl.ocks.org/mbostock/7322386
        -J'ai simplifié le code pour qu'il crée du html plutôt que du svg
        -J'ai organisé le code en fonctions
        -J'ai traduit et ajouté les commentaires
      Voir le tutoriel original pour plus d'info:
      http://bost.ocks.org/mike/bar/


      http://bl.ocks.org/mbostock/raw/1129492/
      http://bl.ocks.org/mbostock/31ce330646fa8bcb7289ff3b97aab3f5
      http://vallandingham.me//bulle_charts_with_d3v4.html
      ...
      http://stackoverflow.com/questions/15532371/do-svg-docs-support-custom-data-attributes

  http://stackoverflow.com/questions/10808320/can-enter-selection-be-reused-after-append-insert
  http://d3-wiki.readthedocs.io/zh_CN/master/Selections/
